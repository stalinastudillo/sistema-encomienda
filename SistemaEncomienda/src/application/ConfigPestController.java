...
//	Fin
	...
	private void initialize(){
		System.out.println("controlador de pesta馻s "+tabPane.getTabs().get(3).getText() );
...
//	Pesta馻 Unidad
	...
	public void actionUnidadAceptar(){
		...
		if (tfuUnidad.getText().compareTo("") != 0 && tfuFactor.getText().compareTo("") != 0 ){
			...
			luAlerta.setVisible(true);luAlerta.setText("Informaci髇 ingresada exitosamente");
		}else{
			luAlerta.setVisible(true);luAlerta.setText("Debe ingresar la informaci贸n solicitada");
		}
	}
//	Fin Pesta馻 Unidad
...
//	Pesta馻 Tarifa
	...
	public void actionTarifaAceptar(){
		...
		if (tftTarifa.getText().compareTo("") != 0){
			...
			closeSesion(sesion);
			tftTarifa.setText("");
			ltAlerta.setVisible(true);ltAlerta.setText("Informaci髇 ingresada exitosamente");
		}else{
			ltAlerta.setVisible(true);ltAlerta.setText("Debe ingresar la informaci贸n solicitada");
		}
	}
//	Fin Pesta馻 Tarifa	
	...
	public void actionVariableAceptar(){
		...
		if (tfvVariable.getText().compareTo("") != 0){
			...
			if (objvc == null){
				...
				closeSesion(sesion);
				tfvVariable.setText("");
				lvAlerta.setVisible(true);lvAlerta.setText("Informaci髇 ingresada exitosamente");
				//irme a la pesta馻 de detalle variable
			}else{
				closeSesion(sesion);
...
//			lvAlerta.setVisible(true);lvAlerta.setText("Informaci髇 ingresada exitosamente");
			...
			lvAlerta.setVisible(true);lvAlerta.setText("Debe ingresar la informaci贸n solicitada");
		}
	}
//	Fin Pesta馻 Variable
...
//	Pesta馻 Ciudad Destino
	...
	public void actionCiudadDestinoAceptar(){
		...
		if (tfcdCiudadDestino.getText().compareTo("") != 0){
			...
			closeSesion(sesion);
			tfcdCiudadDestino.setText("");
			lcdAlerta.setVisible(true);lcdAlerta.setText("Informaci髇 ingresada exitosamente");
		}else{
			lcdAlerta.setVisible(true);lcdAlerta.setText("Debe ingresar la informaci贸n solicitada");
		}
	}
//	Fin Pesta馻 Ciudad Destino
...
//	Pesta馻 Tipo Embalaje
	...
	public void actionTipoEmbalajeAceptar(){
		...
		if (tfteTipoEmbalaje.getText().compareTo("") != 0){
			...
			closeSesion(sesion);
			tfcdCiudadDestino.setText("");
			lteAlerta.setVisible(true);lteAlerta.setText("Informaci髇 ingresada exitosamente");
		}else{
			lteAlerta.setVisible(true);lteAlerta.setText("Debe ingresar la informaci贸n solicitada");
		}
	}
//	Fin Pesta馻 Tipo Embalaje
	...
	private void actionatpBotonMas(){
		...
		try{
			...
			if (itemsPrecioTarifa.isEmpty()){				
				...
				itemsPrecioTarifa = FXCollections.observableArrayList(queryResultPrecioTarifa.list());
				closeSesion(sesion1);
				System.out.println("tama駉 para la tabla " + itemsPrecioTarifa.size());
				tvatpTabla.setItems(itemsPrecioTarifa);
				
			}else if (!itemsPrecioTarifa.isEmpty()){
				latpMsj.setVisible(true);
				latpMsj.setText("Relaci髇 tarifa y precios existentes");
			}	
		}catch(HibernateException e){
			e.printStackTrace();
		}			
...
//	Pesta馻 Peso
	...
	public void contenidoPeso(){
		...
		tvpTablaPT.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<PesoTarifa>(){
			...
			public void changed(ObservableValue<? extends PesoTarifa> arg0, PesoTarifa arg1,PesoTarifa arg2) {
					...
					if (arg2 != null){
						System.out.println("Opci髇 de tabla seleccionada arg2: "+arg2.getDesde()+" "+arg2.getHasta()+" "+arg2.getUndMedida().getDescripcion());
	...
	private void actionpBotonMenos(){
		...
		try{
			...
			lpAlerta.setText("Eliminaci髇 exitosa");
...
//	Pesta馻 Ipostel
	public void contenidoIpostel(){
		...
		tviTablaIpostel.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Ipostel>(){
			...
			public void changed(ObservableValue<? extends Ipostel> arg0, Ipostel arg1,Ipostel arg2) {
					...
					if (arg2 != null){
						System.out.println("Opci髇 de tabla seleccionada arg2: "+arg2.getDesde()+" "+arg2.getHasta()+" "+arg2.getValor()+" "+arg2.getUndMedida().getDescripcion());
	...
	private void actioniBotonMenos(){
		try{
			...
			biEditar.setDisable(false);
			closeSesion(sesion1);
			liAlerta.setText("Eliminaci髇 exitosa");
	...
	private void actioniBotonMas(){
		if (!tfiDesde.getText().equals("") && !tfiHasta.getText().equals("") && !tfiMonto.getText().equals("") 
			...
			liAlerta.setText("Debe ingresar toda la informaci髇");
			liAlerta.setVisible(true);
		}
	}	
	@FXML
